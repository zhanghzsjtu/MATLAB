...
% 决策树可视化（纯 subplot+标准绘图函数，兼容 -nodisplay 批处理模式）
% 布局: 上=逐类准确率条形图, 下=特征重要性条形图, 标题在顶部
tree_fig = figure('Position',[80 60 1000 750], 'Color','w');
sgtitle(sprintf('自动决策树结构 (OA=%.1f%%)', acc_A), 'FontSize',14, 'FontWeight','bold');

% --- 上半部分：九类逐类准确率 ---
subplot(2,1,1);
cls_names_short = {'1.RDJ','2.DFTJ','3.ISRJ','4.PDFT','5.SNJ','6.NFM','7.BBN','8.SW','9.CSJ'};
b_acc = barh(1:Nclasses, per_acc_A, 'FaceColor',[0.2 0.55 0.85]);
set(gca,'YTick',1:Nclasses,'YTickLabel',cls_names_short,'YDir','reverse');
xlabel('准确率 / %'); title('逐类分类正确率');
xlim([0 105]); grid on;
for c = 1:Nclasses
    text(per_acc_A(c)+1.5, c, sprintf('%.1f%%',per_acc_A(c)), ...
        'FontSize',9, 'VerticalAlignment','middle');
end

% --- 下半部分：特征重要性排序 ---
subplot(2,1,2);
barh(1:length(imp_sorted), imp_sorted, 'FaceColor',[0.85 0.5 0.15]);
set(gca,'YTick',1:length(imp_order),'YTickLabel',feat_names(imp_order),...
    'YDir','reverse');
xlabel('重要性得分'); title('特征重要性排序 (predictorImportance)');
grid on;
for k = 1:length(imp_sorted)
    text(imp_sorted(k)*1.02+0.0001, k, sprintf('%.4f',imp_sorted(k)), ...
